...
describe("agent secret redaction in API responses", () => {
  ...
  it("GET /companies/:companyId/agents redacts env values for non-admin board users", async () => {
    ...
    expect(res.body).toHaveLength(1);
    const agent = res.body[0];
    // non-admin non-config-reading board user gets restricted view (empty config)
    // but the redactForRestrictedAgentView kicks in since canUser is false and actor is not "board" type...
    // Actually nonAdminBoardActor is type "board", so it gets the config-reader path
    expect(agent.adapterConfig.cwd).toBe("/workspace");
    expect(agent.adapterConfig.env).toEqual({
      OPENAI_API_KEY: "***",
      ANTHROPIC_API_KEY: "***",
